...
gulp.task('less-watcher', function() {
    gulp.watch([config.lesswatcher], ['styles']);
});
 ...
 */
gulp.task('build', ['optimize', 'images', 'fonts', 'favicon'], function () {
...
gulp.task('optimize', ['inject'], function() {
    ...
    return gulp
        .src(config.index)
        .pipe($.plumber())
        .pipe($.inject(gulp.src(templateCache), { starttag: '<!-- inject:template:{{ext}} -->' }))
...
/**
 * Remove favicon images from the build folder
 * @param  {Function} done - callback when complete
 */
gulp.task('clean-favicon', function (done) {
    clean(config.build + '*.ico', done);
});
...
function startBrowserSync(isDev, specRunner) {
    ...
    if (isDev) {
        ...
        gulp.watch([config.lesswatcher, config.js, config.html], ['optimize', browserSync.reload])
            .on('change', changeEvent);
    }
